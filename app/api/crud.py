...
def get(db: Session, url: schemas.EncodedUrlSchema):
    return db.query(models.Url).filter(models.Url.encoded_url == url.encoded_url).first()

def get_all_url(db: Session, url: schemas.EncodedUrlSchema):
    return db.query(models.Url).filter(models.Url.encoded_url == url.encoded_url).all()

def post(db: Session, url: schemas.ActualUrlSchema):
    # hashing url
    query = db.query(models.Url).filter(models.Url.actual_url == url.actual_url).first()
